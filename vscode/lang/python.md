...
- No mutable default arguments. Resources via `with`. `datetime.now(UTC)` over `utcnow()`. CPU-bound → `ProcessPoolExecutor` (GIL serializes threads unless free-threaded build, 3.14+).
